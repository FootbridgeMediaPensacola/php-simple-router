...
$("#upload-image-form").on("submit",function(a){a.preventDefault();a=new FormData(this);droppedImage&&a.set("uploaded-image",droppedImage);$.ajax({type:"post",url:"/Controllers/uploadImage.php",data:a,cache:!1,contentType:!1,processData:!1,success:function(a){$("#upload-image-form .upload-image-preview").removeAttr("src");$("#upload-tags-container .image-tag-input-box").remove();$("#image-file-type").html("");$("#image-file-dimensions").html("");$("#image-success").css("opacity",1).removeClass("d-none").animate({opacity:0},
1E3,function(){$("#image-success").addClass("d-none")})}});console.log(a)});
